...
public class Program {
			...
			switch(choose) {
				...
				case 4:{
					if(authorList.isEmpty()) {
						System.out.println("Khong co thang nao het.");
					}else {
						System.out.println("Nhap nick name di ban:");
						String key;
						key = scanner.nextLine();
						boolean isCheck = false;
						for(int i = 0; i <= bookList.size()-1;i++) {
							if(key.equalsIgnoreCase(bookList.get(i).getNickname())) {
								bookList.get(i).showInfor();
								isCheck = true;
							}
						}
						if(!isCheck) {
	...
	public static void showMenu() {
		...
		System.out.println("4.Search book by nickname author.");
